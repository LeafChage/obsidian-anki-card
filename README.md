...
- **Not optimized for small screens** — the card uses a fixed `h-[560px]` and the layout isn't responsive, so on smaller phones the card plus buttons can overflow the viewport. It runs on Obsidian mobile (`isDesktopOnly: false`, no desktop-only APIs), but the layout still needs responsive sizing.
